  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] text-slate-900 antialiased rounded-[2.5rem] p-4 sm:p-6 md:p-6 lg:p-8 xl:p-10">
      <div className="w-full max-w-full md:max-w-3xl lg:max-w-5xl xl:max-w-6xl mx-auto">
        ...
        {/* Top Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 p-6 md:p-8 bg-slate-900 rounded-[2.5rem] border border-slate-700 shadow-2xl shadow-slate-900/20">
          <div className="flex items-center gap-4">
            <div className="p-3.5 bg-slate-700/50 rounded-2xl border border-slate-600 shadow-inner group transition-all hover:border-indigo-400">
              <LiaUsersSolid className="w-7 h-7 md:w-8 md:h-8 text-indigo-300 group-hover:scale-110 transition-transform" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight text-white uppercase">
                All <span className="text-indigo-300 font-light italic">Users</span>
              </h1>
              <div className="flex items-center gap-2 mt-1">
                <span className="flex h-2 w-2 rounded-full bg-indigo-400 animate-pulse"></span>
                <p className="text-xs md:text-sm text-slate-300 font-medium">
                  Total Members: <span className="text-white font-bold tracking-wider">{users.length}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-72 group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-indigo-300 transition-colors" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search identities..."
                className="w-full pl-12 pr-4 py-3 bg-slate-900/50 border border-slate-700 rounded-2xl text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-400/10 transition-all font-medium"
              />
            </div>

            <button
              onClick={() => setAddingUser(true)}
              className="p-3.5 bg-indigo-400 text-slate-900 rounded-2xl hover:bg-white hover:text-indigo-600 transition-all shadow-lg shadow-indigo-400/20 active:scale-90 group"
              title="Add New User"
            >
